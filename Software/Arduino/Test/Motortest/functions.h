...
void singleMotor(int p, boolean d) // Achtung neu !!!
{
  digitalWrite(DRIVE_ENA, HIGH);

  ledcWrite(DRIVE1_PWM, p);
  digitalWrite(DRIVE1_DIR, d);
}
...
void drive(int dir, int speed, int rot) 
{
  ...
  spd_1 = sin(winkel - PI/3.0) * speed;   // vorne links
  spd_2 = sin(winkel + PI/3.0) * speed;   // vorne rechts
  spd_3 = sin(winkel - 3.0*PI/4.0) * speed; // hinten links
