...
{
  rot = BOUND(rot, -MAX_ROTATION, MAX_ROTATION);
  // if rot (mit rot delta) größer als ein bestimmter wert dann wird fahr vektor * 0.3 gerechent
  ...
  spd_1 = sin(angle + PI/3.0);   // vorne links
  spd_2 = sin(angle - PI/3.0);   // vorne rechts
  spd_3 = sin(angle + 3.0*PI/4.0); // hinten links
  spd_4 = sin(angle - 3.0*PI/4.0); // hinten rechts
